...
function inputData(data) {
    $.each(data, function(key, value) {
        if (value['bloodlipid']) {
            ...
            $(className).append('<div class="my-date">' + value['bloodlipid'] + ' mmol/L</div>');
        };
    })
}
...
// 传入今日的血脂值
function changeData() {
    var bloodlipidData = $("#inputData").val()
    if (checkData(bloodlipidData)) {
        $('#myModal').modal('hide');
        $('#helpBlock').removeClass('warning');
        var yearAndMonth = getYearAndMonthOfCurrentPage();
        var day = $($(".today .day-contents")[0]).text();
        var postData = {};
        postData["bloodlipid"] = bloodlipidData;
        postData["year"] = yearAndMonth["year"];
        postData["month"] = yearAndMonth["month"];
        postData["day"] = day;
        $.post("/userrecord/update_bloodlipid", postData, function(data) {
            $(".today .my-date").remove();
            $(".today").append('<div class="my-date">' + bloodlipidData + ' mmol/L</div>');
            alert(data);
        })
    } else {
